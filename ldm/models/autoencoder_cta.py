...
class AutoencoderKLInvariance(AutoencoderKL):
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            # train encoder+decoder+logvar
            aeloss_source, log_dict_ae_source = self.loss(inputs_source, reconstructions_source, posterior_source, optimizer_idx, self.global_step,
                                            last_layer=self.get_last_layer(), split="train")
            aeloss_target, log_dict_ae_target = self.loss(inputs_target, reconstructions_target, posterior_target,
        ...
        if optimizer_idx == 1:
            # train the discriminator
            discloss_source, log_dict_disc_source = self.loss(inputs_source, reconstructions_source, posterior_source, optimizer_idx, self.global_step,
                                                last_layer=self.get_last_layer(), split="train")
            discloss_target, log_dict_disc_target = self.loss(inputs_source, reconstructions_source, posterior_source,
                                                              optimizer_idx, self.global_step,
                                                              last_layer=self.get_last_layer(), split="train")
